...
// ExampleNewAdapter demonstrates how to use the AuthZEN adapter as a drop-in replacement for the Cerbos client.
// The adapter implements cerbos.Client interface and uses Cerbos types (Principal, Resource).
func ExampleNewAdapter() {
	// Create an HTTP adapter
	adapter, err := authzen.NewAdapter("https://pdp.example.com:3592", authzen.WithTLSInsecure())
	if err != nil {
		log.Fatalf("Failed to create adapter: %v", err)
	}

	// Use standard Cerbos types
	principal := cerbos.NewPrincipal("john").
		WithRoles("employee").
		WithPolicyVersion("20210210").
		WithAttributes(map[string]any{
	...
	// Check permission using the same API as cerbos.GRPCClient
	allowed, err := adapter.IsAllowed(context.TODO(), principal, resource, "view")
	if err != nil {
		log.Fatalf("Failed to check permission: %v", err)
	}
...
// ExampleNewGRPCAdapter demonstrates how to use the AuthZEN gRPC adapter.
func ExampleNewGRPCAdapter() {
	// Create a gRPC adapter
	adapter, err := authzen.NewGRPCAdapter("localhost:3593", cerbos.WithPlaintext())
	if err != nil {
	...
	principal := cerbos.NewPrincipal("john").
		WithRoles("employee").
		WithAttributes(map[string]any{"department": "marketing"})

	resource := cerbos.NewResource("leave_request", "XX125").
		WithAttributes(map[string]any{"owner": "john"})
...
// ExampleAdapter_WithPrincipal demonstrates how to use the adapter with a principal context.
func ExampleAdapter_WithPrincipal() {
	adapter, err := authzen.NewAdapter("https://pdp.example.com:3592", authzen.WithTLSInsecure())
	if err != nil {
		log.Fatalf("Failed to create adapter: %v", err)
	}

	// Create a principal context for reuse across multiple checks
	principal := cerbos.NewPrincipal("john").
		WithRoles("employee").
		WithAttributes(map[string]any{"department": "marketing"})

	pc := adapter.WithPrincipal(principal)

	// Check multiple resources with the same principal
	resources := []struct {
		kind   string
		id     string
		action string
	}{
		{"leave_request", "XX125", "view"},
		{"leave_request", "XX125", "approve"},
		{"expense_report", "EXP001", "submit"},
	}

	for _, r := range resources {
		resource := cerbos.NewResource(r.kind, r.id)
		allowed, err := pc.IsAllowed(context.TODO(), resource, r.action)
		if err != nil {
			log.Fatalf("Failed to check permission: %v", err)
		}
		log.Printf("Is john allowed to %s %s/%s: %t", r.action, r.kind, r.id, allowed)
	}
}

// ExampleAdapter_CheckResources demonstrates how to check multiple resources in a batch.
func ExampleAdapter_CheckResources() {
	adapter, err := authzen.NewGRPCAdapter("localhost:3593", cerbos.WithPlaintext())
	if err != nil {
		log.Fatalf("Failed to create adapter: %v", err)
	}
	...
	// Create a batch of resources to check
	...
	batch.Add(
		cerbos.NewResource("leave_request", "XX225").
			WithAttributes(map[string]any{"owner": "mary"}),
		"view",
	)

	result, err := adapter.CheckResources(context.TODO(), principal, batch)
	if err != nil {
		log.Fatalf("Failed to check resources: %v", err)
	}

	// Get result for a specific resource
	xx125 := result.GetResource("XX125")
	if xx125 != nil {
		log.Printf("XX125 view allowed: %t", xx125.IsAllowed("view"))
		log.Printf("XX125 approve allowed: %t", xx125.IsAllowed("approve"))
	}
}
